	...
	public Results classify(ArrayList<Example> examples) {
		...
		if (examples.size()!=0) {
			...
			if (allAgree) {
				...
				ArrayList<ArrayList<Double>> idealFeature = new ArrayList();
				...
				for (int i = 1; i < Feature.featureNames.length; i++) {
					ArrayList<Double> entropyValues = new ArrayList();
//				System.out.println(Feature.featureNames[i]);
...
//					System.out.println("Value="+examples.get(j).numericals[i]);
						ArrayList<Integer> targets = new ArrayList();
						for (int k = 0; k < examples.size(); k++) {
							// Calculate entropy with feature i and value j (< or = depends) and select value with least entropy (APPLY)
							Feature f = new Feature(i, examples.get(j).numericals[i]);   //???????????????????????????????? (numericals)
//						System.out.println(f.apply(examples.get(k)));
							targetValues.add(f.apply(examples.get(k)));


							if (f.apply(examples.get(k)))
								yesList.add(Integer.parseInt(examples.get(k).strings[0]));
							else noList.add(Integer.parseInt(examples.get(k).strings[0]));
						}
					...
					temp.add((double) i);
					temp.add(min);
					temp.add(examples.get(index).numericals[i]);
					idealFeature.add(temp);
				}

				Double minEnt = idealFeature.get(0).get(1);
				Double featureIndex = idealFeature.get(0).get(0);
				Double thresholdValue = idealFeature.get(0).get(2);

				for (int z = 1; z < idealFeature.size(); z++) {
					if (idealFeature.get(z).get(1) < minEnt) {
						featureIndex = idealFeature.get(z).get(0);
						thresholdValue = idealFeature.get(z).get(2);
					}
				}

//				featureIndex = featureIndex.intValue();
				Feature f = new Feature(featureIndex.intValue(), thresholdValue);
				this.feature = f;
				...
				for (int i = 0; i < examples.size(); i++) {
					if (examples.get(i).numericals[featureIndex.intValue()] < thresholdValue) {//?????????????????????
